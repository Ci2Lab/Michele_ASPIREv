...
def _window_2D(window_size, power=2, plot=False):
    """
    ...
    if key in cached_2d_windows:
        ...
        wind = np.expand_dims(np.expand_dims(wind, axis=1), axis=1)
...
def _pad_img(img, window_size, subdivisions, plot=False):
    """
    Add borders to img for a "valid" border pattern according to "window_size" and
    "subdivisions".
    # subdivisions: minimal amount of overlap for windowing. Must be an even number
    Image is an np array of shape (x, y, nb_channels).
    """
    aug = int(round(window_size * (1 - 1.0/subdivisions)))
    more_borders = ((aug, aug), (aug, aug), (0, 0))  #((row pads: top-bottom, col pads: left-right, channel pads: depth=0))
...
def _rotate_mirror_undo(im_mirrs):
    ...
    return np.mean(origs, axis=0).astype(img_type)



def dummy_predict_for_patches(small_img_patches):
        """
        Apply prediction on images arranged in a 4D array as a batch.
        Here, we use a 'dummy' prediction function. It assigns a random color for each patch 
        so as to see how it will blend.
...
def _windowed_subdivs(padded_img, window_size, subdivisions, nb_classes, pred_func):
    ...
    subdivisions: overlapping size between patches. 
                minimal amount of overlap for windowing. Must be an even number
                subdivisions=2 means 50% overlap between patches
    ...
    gc.collect()
    print("Number of patches: " + str(a*b))
    
    # Predict for the tiles
    subdivs = pred_func(subdivs)
    gc.collect()
    
    # Multi-class classification
    subdivs = subdivs.reshape(a*b, c,d, nb_classes)  # --> (N_patches, 80,80, N_classes)
    
    # Weight the prediction in the center of the window more than the edges
...
def predict_img_with_smooth_windowing(input_img, window_size, subdivisions, nb_classes, pred_func):
    ...
    padded_result = _recreate_from_subdivs(
        sd, window_size, subdivisions,
        padded_out_shape=list(pad.shape[:-1])+[nb_classes])

    prd = _unpad_img(padded_result, window_size, subdivisions)
...
#     window_size = 80
#     subdivisions = 2
#     img = np.array(Image.open('area_example.png'))/255
    
#     prd = predict_img_with_smooth_windowing(img, window_size, subdivisions, nb_classes=3, pred_func=dummy_predict_for_patches)
#     plt.imshow(prd)
