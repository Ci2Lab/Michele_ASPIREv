...
def pansharpen(m, pan, psh, R = 1, G = 2, B = 3, NIR = 4, method = 'simple_brovey', W = 0.1):
    ...
    """
    def _stretch(bands, lower_percent=2, higher_percent=98):
    ...
    ges.io.export_GEOtiff("_data_Lillisand/output.tif", img_psh, metadata_pan)
    return img_psh
   
    

if __name__ == "__main__":
    m = "_data_Lillisand/ms_RGBNIR.tif"
    pan = "_data_Lillisand/pan.tif"
    psh = "_data_Lillisand/pan_output.tif"
    output = pansharpen(m, pan, psh, R = 1, G = 2, B = 3, NIR = 4, method = 'simple_brovey', W = 0.1)
    

    processed = ges.geo_utils.preProcessImage("_data_Lillisand/output.tif", NDVI = False, save = True)
