...
class nDSMmodeler(RSClassifier.RSClassifier):
    ...
    def generate_nDSM(self, image, MEMORY_LIMIT = 5, verbose = True, tmp_folder = "tmp"):
        if self.DLmodel is not None:
            ...
            if ESTIMATED_MEMORY < MEMORY_LIMIT:
                ...
                stiched_pred_map = self._generate_nDSM(image,  window_size, subdivisions, verbose = verbose)
            else:
                ...
                if utils.confirmCommand("Image too large, split it?"):
                    ...
                    for i in range(0, N):
                        ...
                        stiched_pred_map.append(self._generate_nDSM(part, window_size, subdivisions, verbose = verbose))
                        # Save the mask
    ...
    def _generate_nDSM(self, image, window_size, subdivisions, verbose = True):                    
            image = (image / 255).astype('float32')
            if verbose:
                print("--Estimating height over tiles (with smoothing function)")
